package com.airhttp
{
    ...
    * 
    ...
    public class HttpServer
    {
        ...
        public function HttpServer( htdocs:File )
        {
            _fileController = new FileController(htdocs);    
        }
        ...
        private function socketDataHandler(event:ProgressEvent):void
        {
            try
            {
                ...
                if (request.substring(0, 3).toUpperCase() != 'GET') {
                  socket.writeMultiByte(ActionController.responseNotAllowed("HttpServer only supports GET requests."), CTOptions.charset)
                  socket.flush();
                  socket.close();
                  return;
                }
                ...
                if (controller) {
                    ...
                    socket.writeMultiByte(controller.doAction(action_key, new URLVariables(param_string)), CTOptions.charset);
                }
                else {
                   // socket.writeMultiByte(_fileController.getFile(url), CTOptions.charset);
